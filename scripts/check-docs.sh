...
check_api_docs() {
    ...
    local endpoints=$(grep -r "@RequestMapping\|@GetMapping\|@PostMapping\|@PutMapping\|@DeleteMapping" src/main/java --include="*.java" | wc -l)
    local documented_endpoints=$(grep -c "GET\|POST\|PUT\|DELETE" API_DOCUMENTATION.md || true)
    
    if [[ $endpoints -gt $documented_endpoints ]]; then
        print_status "WARNING" "Possíveis endpoints não documentados (encontrados: $endpoints, documentados: $documented_endpoints)"
        issues=$((issues + 1))
    fi
...
# Function to check frontend documentation
check_frontend_docs() {
    local issues=0
    
    print_status "INFO" "Verificando documentação do frontend..."
    
    # Check if all components are documented
    local components=$(find frontend/src/components -name "*.tsx" -o -name "*.ts" | grep -v ".test." | wc -l)
    local documented_components=$(grep -c "export.*Component\|export.*const.*:" FRONTEND_DOCUMENTATION.md || true)
    
    if [[ $components -gt $documented_components ]]; then
        print_status "WARNING" "Possíveis componentes não documentados (encontrados: $components, documentados: $documented_components)"
        issues=$((issues + 1))
    fi
    
    # Check if all hooks are documented
    local hooks=$(find frontend/src/hooks -name "*.ts" | wc -l)
    local documented_hooks=$(grep -c "use[A-Z]" FRONTEND_DOCUMENTATION.md || true)
    
    if [[ $hooks -gt $documented_hooks ]]; then
        print_status "WARNING" "Possíveis hooks não documentados (encontrados: $hooks, documentados: $documented_hooks)"
        issues=$((issues + 1))
    fi
...
# Function to check architecture documentation
check_architecture_docs() {
    local issues=0
    
    print_status "INFO" "Verificando documentação de arquitetura..."
    
    # Check if Docker files are documented
    if [[ -f "Dockerfile" ]] && ! grep -q "Dockerfile" ARCHITECTURE.md; then
        print_status "WARNING" "Dockerfile não mencionado na documentação de arquitetura"
        issues=$((issues + 1))
    fi
    
    # Check if docker-compose is documented
    if [[ -f "docker-compose.yml" ]] && ! grep -q "docker-compose" ARCHITECTURE.md; then
        print_status "WARNING" "docker-compose.yml não mencionado na documentação de arquitetura"
        issues=$((issues + 1))
    fi
